...
function is_plugin_loaded(name) return require("lazy.core.config").plugins[name]._.loaded ~= nil end
...
function insert_set(tab, elem)
  if is_contained_in(tab, elem) then return end
  table.insert(tab, elem)
end

function startswith(str)
  return function(start) return str:sub(1, #start) == start end
end

function filter(tab, p)
  local result = {}
  for _, v in ipairs(tab) do
    if p(v) then table.insert(result, v) end
  end
  return result
end
