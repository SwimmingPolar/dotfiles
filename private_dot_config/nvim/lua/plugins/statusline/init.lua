...
return {
  ...
  opts = function()
    ...
    local Navic = {
      condition = function() return is_plugin_loaded("nvim-navic") and require("nvim-navic").is_available() end,
      update = "CursorMoved",
      Space(2),
      {
        provider = function()
    ...
    local Git = {
      ...
      flexible = 40,
      {
        Space(2),
        {
          provider = function(self) return " " .. truncate(self.status_dict.head, 10) end,
        },
      },
      Empty,
    }
    ...
    local SearchCount = {
      ...
      provider = function(self) return string.format("[%d/%d]", self.search.current, self.search.total) end,
      Space(2),
    }
